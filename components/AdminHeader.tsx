...
import { Menu, Search, Bell, ChevronRight, CheckCircle, ShieldCheck, UserCog, Eye } from 'lucide-react';
...
export const AdminHeader: React.FC<AdminHeaderProps> = ({
  ...
  notificationRef, unreadCount, role
}) => {
  const RoleIcon = role === 'SUPER_ADMIN' ? ShieldCheck : role === 'EDITOR' ? UserCog : Eye;
    ...
    <header className="h-24 border-b border-slate-800/30 bg-slate-900/20 backdrop-blur-2xl flex items-center justify-between px-10 shrink-0 relative z-50">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-4 pl-6 border-l border-slate-800">
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-white text-xs uppercase border border-white/5 transition-all shadow-lg ${role === 'SUPER_ADMIN' ? 'bg-blue-600 shadow-blue-500/20' : role === 'EDITOR' ? 'bg-amber-600 shadow-amber-500/20' : 'bg-slate-700 shadow-slate-900/40'}`}>
