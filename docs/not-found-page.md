...
| Language | French, aligned with the rest of the site |
...
| `src/lib/notFound.ts` | Page content (destinations, diagnostic checks) |
| `src/types/notFound.ts` | `NotFoundDestination`, `NotFoundCheck` contracts |
...
Destinations and diagnostic checks are declared once in `src/lib/notFound.ts`. The latest
reads list derives from `blogPosts` (`src/lib/portfolio.ts`) sorted by `date` descending,
capped by `LATEST_READS_COUNT`. No content is hardcoded in JSX.
...
- `notFound()` from `/blog/[slug]` and `/projets/[slug]`.
